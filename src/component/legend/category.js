...
const { DomUtil, Event, Group } = require("@ay/g-node");
...
class Category extends Base {
  getDefaultCfg() {
    ...
    return Util.mix({}, cfg, {
      ...
      _defaultItemTpl:
        '<li class="' +
        ITEM_CLASS +
        ' item-{index} {checked}" data-color="{originColor}" data-value="{originValue}">' +
        '<i class="' +
        MARKER_CLASS +
  ...
  _onClick(ev) {
    ...
    if (clickedItem && !clickedItem.get("destroyed")) {
      ...
      if (
        !this.get("allowAllCanceled") &&
        checked &&
        this.getCheckedCount() === 1
      ) {
        return;
      }
      ...
      if (mode === "single") {
        ...
        if (markerItem.attr("stroke")) {
          markerItem.attr(
            "stroke",
            checked ? unCheckColor : item.marker.stroke
          );
        }
  ...
  _renderHTML() {
    ...
    DomUtil.modifyCSS(
      itemListDom,
      Util.mix({}, LEGEND_STYLE[LIST_CLASS], self.get(LIST_CLASS))
    );
    ...
    if (titleDom) {
      if (title && title.text) {
        ...
        DomUtil.modifyCSS(
          titleDom,
          Util.mix({}, LEGEND_STYLE[TITLE_CLASS], self.get(TITLE_CLASS))
        );
      } else {
    ...
    const itemStyle = Util.mix(
      {},
      LEGEND_STYLE[ITEM_CLASS],
      {
        display:
          position === "right" || position === "left" ? "block" : "inline-block"
      },
      self.get(ITEM_CLASS)
    );
    const markerStyle = Util.mix(
      {},
      LEGEND_STYLE[MARKER_CLASS],
      self.get(MARKER_CLASS)
    );
    ...
    if (self.get("clickable")) {
      ...
      legendWrapper.onclick = ev => {
        ...
        if (
          targetClass.indexOf(CONTAINER_CLASS) > -1 ||
          targetClass.indexOf(LIST_CLASS) > -1
        ) {
          return;
        }
        ...
        const clickedItem = findItem(
          items,
          parentDom.getAttribute("data-value")
        );
        ...
        if (mode === "single") {
          ...
          Util.each(childNodes, child => {
            if (child !== parentDom) {
              ...
              parentDom.className = Util.replace(
                domClass,
                "unChecked",
                "checked"
              );
    ...
    if (/^\#/.test(container)) {
      ...
      DomUtil.modifyCSS(
        ...
        Util.mix(
          {},
          LEGEND_STYLE[CONTAINER_CLASS],
          rangeStyle,
          self.get(CONTAINER_CLASS)
        )
      );
