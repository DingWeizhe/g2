...
const { Canvas, Event } = require("@ay/g-node");
...
describe("分类图例", function() {
  ...
  it("html 渲染图例，使用默认的模板", function() {
    ...
    expect(legendItem.className).to.equal(
      "g2-legend-list-item item-1 unChecked"
    );
    ...
    const hoveredLegendItem = div.getElementsByClassName(
      "g2-legend-list-item"
    )[2];
  ...
  it("html 渲染图例，使用回调函数自定义模板", function() {
    ...
    const legend = canvas.addGroup(Legend, {
      ...
      itemTpl(value, color) {
        const tpl =
          '<li class="g2-legend-list-item item-{index} {checked}" data-color="{originColor}" data-value="{originValue}" style="cursor:pointer;display: inline-block;width: 85px">' +
          ...
          '<span class="g2-legend-text" style="color:' +
          color +
          '">' +
          value +
          "</span></li>";
    ...
    expect(legendItem10.className).to.equal(
      "g2-legend-list-item item-10 checked"
    );
    expect(legendItem11.className).to.equal(
      "g2-legend-list-item item-11 unChecked"
    );
    // 模拟点击事件
    const event = new MouseEvent("click", {
      view: window,
    ...
    expect(legendItem10.className).to.equal(
      "g2-legend-list-item item-10 unChecked"
    );
    expect(legendItem11.className).to.equal(
      "g2-legend-list-item item-11 checked"
    );
    div.removeChild(legendDom);
  });
  ...
  it("html 渲染图例，使用字符串自定义模板", function() {
    ...
    const legend = canvas.addGroup(Legend, {
      ...
      position: "bottom",
      itemTpl:
        '<li class="g2-legend-list-item item-{index} {checked}" data-color="{originColor}" data-value="{originValue}" style="cursor:pointer;width: 85px"><span class="g2-legend-text" style="color: {color};cursor: pointer;">{value}</span></li>'
    });
    legend.move(0, 0);
    canvas.draw();
    ...
    expect(legendItem01.className).to.equal(
      "g2-legend-list-item item-1 checked"
    );
    ...
    expect(legendItem00.className).to.equal(
      "g2-legend-list-item item-0 unChecked"
    );
    // 模拟点击事件2
    ...
    expect(legendItem00.className).to.equal(
      "g2-legend-list-item item-0 checked"
    );
  });
});
