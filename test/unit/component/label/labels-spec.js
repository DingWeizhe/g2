...
describe("HTML 自定义 Labels", function() {
  it("html 渲染，htmlTemplate 是个字符串", function() {
    ...
    const labels = canvas.addGroup(Labels, {
      ...
      htmlTemplate:
        '<p style="color: red;font-weight: 700;font: 12px arial;text-align: center;width: 50px;">{text}</p>',
      formatter(value) {
        return "text--" + value;
      }
    });
    ...
    expect(lastNode.innerHTML).to.equal(
      '<p style="color: red;font-weight: 700;font: 12px arial;text-align: center;width: 50px;">text--2</p>'
    );
  ...
  describe("自定义 Labels，回调", function() {
    ...
    it("labels add Labels By Object", function() {
      ...
      const lastNode = childNodes[childNodes.length - 1];
      expect(lastNode.innerHTML).to.equal(
        '22<span style="font-size: 30px;color: yellow;">2122121</span>9'
      );
    });

    it("custom renderer", function() {
      const item = customDiv.childNodes[0];
      expect(item.innerHTML).to.equal(
        '10<span style="font-size: 30px;color: yellow;">1</span>0'
      );
    });
    ...
    it("change label", function() {
      ...
      expect(label.innerHTML).to.equal(
        '100<span style="font-size: 30px;color: yellow;">变化</span>0'
      );
    });
